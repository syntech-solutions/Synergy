...
async function getNameFromDb(userId) {
  try {
    ...
    const q = query(
      collection(db, "userDetails/" + userId + "/chats").withConverter(
        nameConverter
      )
    );
...
function ChatPeopleDescription(props) {
  return (
    <Typography
      sx={{ fontSize: "92.5%", fontFamily: "Calibri", textTransform: "none" }}
      align="left"
    >
      {props.name}
    </Typography>
...
const ChatPeople = () => {
  ...
  const handleClicky2 = async () => {
    ...
    const p = new Map([
      [id1, [name1, profilePic1]],
      [id2, [name2, profilePic2]],
    ]);
    ...
    onSnapshot(qchatColl, (querySnapshot) => {
      querySnapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
          if (change.doc.id !== "chatID") {
            //check also if logged in user is in participants
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <Typography sx={{ fontSize: 16 }}>New Chat</Typography>
          </Button>
